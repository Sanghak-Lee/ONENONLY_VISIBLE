...
(function ($) {
	...
	if ($("body").hasClass("tt-transition")) {
		$(window).on("pageshow", function (event) {
			if (event.originalEvent.persisted) { //recover from BFCache(뒤로가기)
				$(function(){
					setTimeout(function(){
						HideLoad(); // call out animations.
						var re_tl = gsap.timeline({ defaults: { duration: 1.5, ease: Expo.easeInOut }});
						re_tl.to("#content-wrap", { y: -80, autoAlpha: 1 }, 0);
					}, 0)
				});
			}
			else { //새로열린 페이지
   ...
   // ============================================================================
	...
	$(".pli-info").each(function() {
		...
		if ($($pliTag).length) {
			tl_plIInfo.from($pliTag, { duration: 2.5, autoAlpha: 0, y: 70, ease: Expo.easeOut, clearProps:"all" }, "-=2.2");
		}				
